"""
Standalone t-SNE re-plotting script.

After a training run, analysis.py saves a *_tsne_features_*.npz file
containing raw features, true labels, and predicted labels.
This script re-draws the t-SNE plots from that saved npz file,
so you can tweak visualization parameters WITHOUT re-running training/evaluation.

Usage:
    python utils/replot_tsne.py \
        --npz  /path/to/analysis/hmmem-qwen-meld-20260604_120000/hmmem-qwen-meld-TEST-tsne_features.npz \
        --labels neutral surprise fear sadness joy disgust anger \
        --tag  "hmmem-qwen-meld-TEST" \
...
from utils.analysis import (
    _plot_emotion_tsne, _plot_confusion_matrix, _save_multi_format,
    logger,
)
...
def main():
    parser = argparse.ArgumentParser(
        description='Re-plot t-SNE / Confusion Matrix from saved npz features'
    )
    parser.add_argument('--npz', type=str, required=True,
                        help='Path to *_tsne_features_*.npz saved by analysis.py')
    ...
    parser.add_argument('--max_per_class', type=int, default=150,
                        help='Max samples per emotion class for balanced t-SNE (default: 150)')
    parser.add_argument('--confusion_matrix', action='store_true',
                        help='Also re-plot confusion matrix')
    ...
    # Tag: infer from filename  e.g. "hmmem-qwen-meld-TEST-tsne_features-20260604.npz"
    if args.tag is None:
        basename = os.path.basename(args.npz)
        # Remove "-tsne_features*" suffix
    ...
    # ── Optional: Re-plot Confusion Matrix ──
